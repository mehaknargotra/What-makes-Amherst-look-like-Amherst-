...
def get_classtable():
    classes = []
    with open("./code/test_grad_cam/synset_words.txt") as lines:
        for line in lines:
...
#----------------------------GRADCAM For ResNet-152-------------------------------
def demo2(image_paths, output_dir, model, cuda):
    """
...
# This function is created to be used for our project
def run_grad_cam(model, validation_imgs):
    """
    Run Validation Images on Demo2 of Grad CAM
    Need to also substitute model
    """

    demo2(image_paths=validation_imgs, 
          output_dir="./results", 
          model=model, 
          cuda=True)
